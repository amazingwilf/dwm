...
enum { SchemeNorm, SchemeSel, SchemeScratchNorm, SchemeFloat, 
       SchemeScratchSel, SchemeTagsEmpty, SchemeSticky, SchemeTagsOcc, 
       SchemeTagsSel, SchemeLtSym }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			drw_text(drw, x, 0, w, bh, tlpad + (m->sel->icon ? m->sel->icw + ICONSPACING : 0), m->sel->name, 0);
			if (m->sel->icon) drw_pic(drw, x + tlpad, (bh - m->sel->ich) / 2, m->sel->icw, m->sel->ich, m->sel->icon);
			if (m->sel->isfloating)
