...
enum { SchemeNorm, SchemeSel, SchemeFloat, SchemeScratch, SchemeStButton,
       SchemeLtSymbol, SchemeTagsSel, SchemeTagsEmpty, SchemeTag1, 
       SchemeTag2, SchemeTag3, SchemeTag4, SchemeTag5, SchemeTag6 }; /* color schemes */
...
focus(Client *c)
{
	...
	if (c) {
		...
		grabbuttons(c, 1);
		if (c->scratchkey != 0)
			XSetWindowBorder(dpy, c->win, scheme[SchemeScratch][ColBorder].pixel);
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        ...
        XRDB_LOAD_COLOR("dwm.floatfgcolor", floatfgcolor);
        XRDB_LOAD_COLOR("dwm.floatbgcolor", floatbgcolor);
        XRDB_LOAD_COLOR("dwm.floatbordercolor", floatbordercolor);

        XRDB_LOAD_COLOR("dwm.scratchfgcolor", scratchfgcolor);
        XRDB_LOAD_COLOR("dwm.scratchbgcolor", scratchbgcolor);
        XRDB_LOAD_COLOR("dwm.scratchbordercolor", scratchbordercolor);
        ...
        XRDB_LOAD_COLOR("dwm.tag1fgcolor", tag1fgcolor);
        XRDB_LOAD_COLOR("dwm.tag2fgcolor", tag2fgcolor);
        XRDB_LOAD_COLOR("dwm.tag3fgcolor", tag3fgcolor);
        XRDB_LOAD_COLOR("dwm.tag4fgcolor", tag4fgcolor);
        XRDB_LOAD_COLOR("dwm.tag5fgcolor", tag5fgcolor);
        XRDB_LOAD_COLOR("dwm.tag6fgcolor", tag6fgcolor);
      }
    }
  }
...
togglefloating(const Arg *arg)
{
	...
	if (selmon->sel->scratchkey != 0)
		XSetWindowBorder(dpy, selmon->sel->win, scheme[SchemeScratch][ColBorder].pixel);
